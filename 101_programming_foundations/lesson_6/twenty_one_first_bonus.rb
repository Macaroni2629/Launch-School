...
# Calculating the total. You'll notice that in our solution, every time we need the player's or dealer's total value, we call a total method. Doing this is expensive, from a performance perspective (suppose that we cared about performance). Do we really need to calculate the total value over and over? Can we use a local variable to cache the calculation, and instead of calling total every time, just use the local variable? For example, our solution contains multiple calls to total within the main loop:
...
# puts "=============="
# Along with that, total is also being invoked within the busted? and detect_result methods:
