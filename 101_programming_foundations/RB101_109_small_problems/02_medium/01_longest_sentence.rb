...
=begin
Initialize Endings array

Split text on spaces so you have an array of words

Iterate through array of words, word by word
  Check if the ending has a question mark, exclamation point, or period
  If so, record the index of where it does.
...
def longest_sentence(text)
  ...
  array_of_words.each_with_index do |word, index|
    if ENDINGS.include?(word[-1])
      ending_index = index
      result << array_of_words[starting_index..ending_index]
      starting_index = ending_index + 1
    end
  end
  
  sorted_sentences = result.sort_by { |subarray| subarray.size}
  p sorted_sentences.last.size
  sorted_sentences.last.join(" ")
end
