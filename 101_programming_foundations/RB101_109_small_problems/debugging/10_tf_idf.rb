...
Let's take a closer look at our first test case. The term "cat" occurs in 2 of 3 documents, which returns an idf value of Math.log(3/2) => Math.log(1) => 0.
...
The idf value of "cat" is then correctly calculated as Math.log(3.fdiv(2)) => Math.log(1.5) => 0.4054651081081644. The tfidf method multiplies the returned idf by the tf value for document1 (which is 3, because "cat" occurs three times in that document) and returns the expected output of ~1.2 for the first example.
