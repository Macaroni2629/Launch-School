...
This means that when summing up the card scores with remaining_cards.sum on line 34, the remaining_cards array still contains :jack instead of its score 10. (Your error message may vary slightly, naming another face card instead).
...
After the first iteration, you see something like the following (the details will differ for you):
...
The important thing to notice here is that although we intended to apply shuffle! and pop to only one of the arrays (returned by deck[suit]), these method invocations impact all four arrays; they are all shuffled in the exact same way and they all miss the player card we picked (2 in this example).
