...
Discussion
Most of the work with solving this problem comes from having to determine the way we structure our input file. We have opted to use a really neat feature of Kernel#format; this method lets you put strings of the form %{name} inside your format string, and then provide a bunch of keyword arguments. Each of the keyword arguments is matched to one of the %{name} strings, and if the name matches, the value associated with that name is plugged in.

Our main loop does this for each line we read from madlibs.txt; each of the special placeholders %{noun}, %{verb}, etc. is associated with a randomly generated word of the proper type, and then format uses those values to replace each of the placeholders. With each new line, we get a new set of random words, so every line is treated differently.

The main loop uses the DATA pseudo-file as explained above in the Approach/Algorithm section. The rest of the program is just four constant Arrays that contain the candidate words for each word type.
