...
class PokerHand
  def initialize(deck)
    ...
    #   @hand << deck.draw
    # end
    @hand = [Card.new(10, "Heart"), Card.new(10, "Heart"), Card.new(10, "Heart"), Card.new("Jack", "Heart"), Card.new(10, "Heart")]
  end
  ...
  def royal_flush?
    ...
    @hand.all? { |card| card.suit == @hand[0].suit } && ((['Ace', 'King', 'Queen', 'Jack', 10] - @hand.map { |card| card.rank }).empty?)
  end
  ...
  def straight_flush?
    ...
    p value_hands.uniq
    p value_hands
    (value_hands.uniq == value_hands) #&& (value_hands.first..value_hands.last).to_a - value_hands == []
  end

  def four_of_a_kind?
    # Four cards of the same rank and any one other card
    @hand.any? { |card| @hand.count(card.rank) == 4 }
  end
